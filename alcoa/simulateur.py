...
class Garnissage:
    ...
    - substance: objet Substance représentant le garnissage et ses propriétés
    - ratio_volume_solide: fraction du volume occupé par le garnissage
    - vitesse_fluide: m/s
    - hauteur_garnissage: m
    ...
    def demarrer_phase(
        ...
        if len(self.phases) == 0:
            n_points = int(self.hauteur_garnissage / self.resolution_spatiale)
    ...
    def animer_ligne(self, nom_fichier="ligne", step: int = 1):
        ...
        ax.set_xlim(0, self.hauteur_garnissage)
    ...
    def animer_heatmap(self, nom_fichier="heatmap", step: int = 1):
        ...
        print(f"Animation du graphique {nom_fichier}")
        T = self.get_resultats()
        x_min, x_max = 0, self.hauteur_garnissage
        y_min, y_max = 0, 3
        n_y_points = 20
        T_min = np.min(T)
        T_max = np.max(T)
    ...
    def plot_temperatures_at_x(self, fractions: list[float]):
        """
        ...
        for fraction in fractions:
            ...
            ax.plot(
                T[x_point, :], label=f"x = {fraction*self.hauteur_garnissage:.2f} m"
            )
        ax.legend()
...
if __name__ == "__main__":
    ...
    subst_garnissage = Substance(
        temperature=300,  # température initiale du garnissage
        densite=2600,
